...
class BM25PlusEpsilon(object):
    def __init__(self, docs, epsilon=0.25):
        self.D = len(docs)
    ...
    def sim(self, doc, index, average_idf):
        ...
        for word in doc:
            ...
            denominator = self.f[index][word] + self.k1 * \
                (1 - self.b + self.b * self.D / self.avgdl)
...
def bm25pluseps_weights(docs, eps):
    bm25 = BM25PlusEpsilon(docs, epsilon=eps)
    average_idf = sum(
        map(lambda k: bm25.idf[k] + 0.00, bm25.idf.keys())) / len(bm25.idf.keys())
    weights = []
    for doc in docs:
